...
export function DashboardClient({
  userName,
  todayStr,   // ← "YYYY-MM-DD" dari server, timezone-safe
  stats,
  chartData,
  recentTreatments,
}: {
  ...
  const progressPct = hasTarget
    ? Math.min(Math.round((monthCount / targetPatients!) * 100), 100)
    : 0;
  ...
  return (
    <main className="min-h-screen bg-[#183528] px-4 pt-8 pb-28 max-w-lg mx-auto space-y-6 font-[family-name:var(--font-geist-sans)]">
        ...
        <div className="col-span-2 bg-[#F4F3ED] rounded-[1.5rem] p-4 shadow-lg">
          <div className="flex items-start justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[11px] font-bold uppercase tracking-widest text-gray-500 leading-tight">
                  Target Bulan Ini
                </p>
                {hasTarget ? (
                  <p className="text-sm font-bold text-gray-900 mt-0.5">
                    {monthCount}{" "}
                    <span className="text-gray-400 font-medium">/ {targetPatients} pasien</span>
                  </p>
                ) : (
      ...
      <div className="bg-[#F4F3ED] rounded-[2rem] shadow-xl p-5 sm:p-6">
        ...
        {/* Legend */}
        <div className="flex flex-wrap items-center gap-3 sm:gap-4 mb-6 px-1">
          {[
            { color: "bg-[#1D4130]", label: "Hari ini" },
        ...
        <div className="flex items-end justify-between gap-1.5 h-36">
          {chartData.map((day) => {
            ...
            const isToday = day.date === todayStr; // ← server-side string, no timezone issue
            const isFuture = day.date > todayStr;

            const barColor = isToday
              ? "bg-[#1D4130]"
              : day.isOffDay
                ? day.count > 0 ? "bg-red-400" : "bg-red-200"
                : isFuture
                  ? "bg-gray-200"
            ...
            return (
              <div key={day.date} className="flex-1 flex flex-col items-center gap-2">
                <span className="text-[10px] font-bold text-gray-400 h-3">
                ...
                <div className="flex flex-col items-center text-center leading-[1.2] mt-1">
                  ...
                  <span className={`text-[9px] font-bold opacity-60 ${labelColor}`}>
                    {dateObj.getDate()}
                  </span>
                </div>
              </div>
      ...
      <div className="bg-[#F4F3ED] rounded-[2rem] shadow-xl p-5 sm:p-6">
        <div className="flex items-start sm:items-center justify-between mb-5 gap-4">
          ...
          <Link
            href="/rekap"
            className="flex items-center gap-1 text-xs font-bold text-[#1D4130] hover:underline underline-offset-2 transition-colors flex-shrink-0 whitespace-nowrap bg-white/50 px-3 py-1.5 rounded-full"
          >
