...
import { CalendarIcon, XIcon, ChevronDownIcon } from "lucide-react";
...
// Build tab-separated row for spreadsheet paste
function buildCopyRow(row: TreatmentData) {
  const cols = [
    formatDateForCopy(row.date),   // Tanggal
    row.customerName,               // Nama Customer
    row.facialTypeName ?? "",       // Treatment
    row.orderNo,                    // Order No
    row.customerType,               // Tipe Customer
    "",                             // Tanggal opsional (kosong)
    row.nextAppointment ?? "",      // Next Treatment
    row.therapistName,              // Nama Therapist
  ];
  return cols.join("\t");
}

// ─── Date Button (Popover + Calendar) ────────────────────────────────────────
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          type="button"
          className={`w-full h-10 rounded-xl bg-white border flex items-center gap-2 px-3 text-sm transition-all ${
            value
              ? "text-gray-800 border-[#1D4130]/30"
              : "text-gray-400 border-gray-200"
          } focus:outline-none hover:border-[#1D4130]/40`}
        >
    ...
    >
      ...
      <div className="relative w-full max-w-sm bg-[#F4F3ED] rounded-[2rem] overflow-hidden shadow-2xl font-[family-name:var(--font-geist-sans)]">
        ...
        <div className="px-6 pb-6 pt-2 flex gap-2">
          <button
            onClick={() => onCopy(row)}
            className={`flex-1 flex items-center justify-center gap-2 h-11 rounded-2xl text-sm font-bold transition-all duration-150 ${
              copied
          ...
          >
            {copied
              ? <><CheckmarkCircle01Icon size={16} /> Tersalin!</>
              : <><Copy01Icon size={16} /> Copy Baris</>
            }
          </button>
          <button
            onClick={onClose}
            className="h-11 px-4 rounded-2xl border border-gray-200 bg-white text-gray-600 text-sm font-bold hover:bg-gray-50 transition-colors"
          >
            Tutup
          </button>
        </div>
...
export function RecapClient({ initialData }: { initialData: TreatmentData[] }) {
  ...
  }, [initialData, filter, customFrom, customTo]);
  ...
  const handleCopyAll = () => {
    const text = filteredData.map(buildCopyRow).join("\n");
      ...
      <div className="max-w-2xl mx-auto space-y-5">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-[#F4F3ED]/60 mt-1">{filteredData.length} treatment ditemukan</p>
          </div>
            ...
            <div className="flex gap-1.5 overflow-x-auto pb-0.5 scrollbar-hide">
              {filterButtons.map((f) => (
                <button
                  ...
                  onClick={() => {
                    setFilter(f.key);
                    if (f.key === "custom") setShowDatePicker(true);
        ...
        {/* Custom date range picker — shadcn Popover + Calendar */}
        {filter === "custom" && showDatePicker && (
          <div className="bg-[#F4F3ED] rounded-2xl p-4 flex items-end gap-3">
            <div className="flex-1">
              ...
              <DateButton
                ...
                onChange={(v) => {
                  setCustomFrom(v);
                  // Reset "sampai" if it's before the new "dari"
                  if (customTo && v > customTo) setCustomTo("");
                }}
            ...
            <button
              ...
              className="h-10 px-4 rounded-xl bg-[#1D4130] text-[#F4F3ED] text-xs font-bold flex-shrink-0 hover:bg-[#153023] transition-colors"
            >
              Terapkan
            </button>
          </div>
          ...
          <button
            ...
            className={`w-full h-11 rounded-2xl flex items-center justify-center gap-2 text-sm font-bold transition-all duration-150 border ${
              copiedAll
                ...
                : "bg-white/10 text-[#F4F3ED] border-[#F4F3ED]/20 hover:bg-white/15"
            }`}
          >
            {copiedAll
              ? <><CheckmarkCircle01Icon size={16} /> {filteredData.length} baris tersalin!</>
              : <><Copy01Icon size={16} /> Copy Semua ({filteredData.length} baris)</>
            }
          </button>
        )}
        ...
        {filteredData.length === 0 && (
          <div className="bg-[#F4F3ED] rounded-[2rem] px-6 py-14 text-center shadow-xl">
            ...
            <p className="text-xs text-gray-400 mt-1">
              {filter !== "all" ? "Coba ganti filter" : "Tambahkan treatment pertama kamu"}
            </p>
          </div>
        )}
          ...
          <div className="space-y-3">
            {filteredData.map((row) => {
              ...
              return (
                <div
                  key={row.id}
                  className="bg-[#F4F3ED] rounded-[1.5rem] p-4 shadow-lg border border-white/50"
                >
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    <button
                      ...
                      className={`flex-1 h-9 rounded-xl flex items-center justify-center gap-1.5 text-xs font-bold transition-all duration-150 ${
                        isCopied
                          ? "bg-emerald-500 text-white"
                          : "bg-white text-[#1D4130] border border-gray-200 hover:border-[#1D4130]/30"
                      }`}
                    >
                    ...
                    <button
                      onClick={() => setSelectedRow(row)}
                      className="h-9 px-3 rounded-xl bg-white border border-gray-200 text-gray-500 hover:text-[#1D4130] hover:border-[#1D4130]/30 transition-colors flex items-center gap-1.5 text-xs font-bold"
                    >
                ...
                <thead>
                  <tr className="border-b border-gray-200">
                    {["Tanggal", "Order", "Nama", "Tipe", "Treatment", "Next", ""].map((h, i) => (
                      <th key={i} className="text-left px-4 py-3.5 text-[10px] font-bold uppercase tracking-widest text-gray-400 whitespace-nowrap">
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {filteredData.map((row, i) => {
                    ...
                    return (
                      <tr
                        key={row.id}
                        className={`border-b border-gray-100 last:border-0 hover:bg-white/60 transition-colors cursor-pointer ${i % 2 === 1 ? "bg-white/30" : ""}`}
                      >
                        <td className="px-4 py-3 text-xs text-gray-500 whitespace-nowrap" onClick={() => setSelectedRow(row)}>
                        ...
                        <td className="px-4 py-3 whitespace-nowrap" onClick={() => setSelectedRow(row)}>
                          <span className={`inline-flex text-[9px] font-bold px-2 py-1 rounded-full border whitespace-nowrap ${
                            isZEP ? "bg-[#1D4130] text-[#F4F3ED] border-[#1D4130]" : "bg-white text-[#1D4130] border-gray-200"
                          }`}>{row.customerType}</span>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <button
                            onClick={() => handleCopyRow(row)}
                            className={`h-7 px-2.5 rounded-lg flex items-center gap-1 text-[10px] font-bold transition-all ${
                              isCopied ? "bg-emerald-500 text-white" : "bg-white border border-gray-200 text-[#1D4130] hover:border-[#1D4130]/30"
                            }`}
                          >
